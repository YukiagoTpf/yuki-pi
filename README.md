...
- `extensions/plan-mode/index.ts` — `/plan` / plan-mode workflow: read-only research, `plan_write` draft authoring (full/skeleton/patch), `get_plan_mode_status` self-check, automatic review, approval, final plan file, and plan-owned todo seeding.
