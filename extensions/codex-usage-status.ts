...
import { DEFAULT_PREFERENCES, errorMessage, isCodexProvider, type Preferences, type UsageSnapshot } from "../src/codex-usage/domain.ts";
...
class CodexUsageStatus {
	...
	private onModelSelect(ctx: ExtensionContext, model: { id: string; provider: string }): void {
		...
		void this.refresh(ctx, model.id);
	}
	...
	private renderLast(ctx: ExtensionContext): boolean {
		if (!ctx.hasUI || !this.lastUsage || !isCodexProvider(ctx.model?.provider)) return false;
