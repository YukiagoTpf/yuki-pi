...
- 先调研 pi / pi-ai runtime 是否暴露 tool 参数校验失败 hook，或是否能统一改进 validation error 的格式化。
- 在 hook 可用前，Incident 1 的 plan-flow 侧真修复是 B2（steering 预算，预防）与 C2（skeleton/patch，结构性）。
...
2. A1 降级为 runtime validation hook 调研项：确认 pi / pi-ai 是否允许扩展或核心运行时重写 schema validation error；确认前不承诺为 plan-flow 内部交付项，也不阻塞 C2。

这吸收修正后的 B2 + A1。B2 是近期可控修复；A1 只有在 runtime hook 可用时才进入交付，优先级低于 C2 的结构性 API 演进。
...
7. **A1 截断诊断：降级为 runtime validation hook 调研项，可行后再交付。**
